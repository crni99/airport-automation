...
namespace AirportAutomation.Api.Controllers
{
	...
	public class FlightsController : BaseController
	{
		...
		[ProducesResponseType(401)]
		public Task<ActionResult<FlightDto>> GetFlight(int id)
			=> GetByIdAsync<FlightEntity, FlightDto>(
				_cacheService, _inputValidationService, _mapper, _logger,
				id,
				cacheKey: CacheKeys.Flight(id),
				fetchItem: () => _flightService.GetFlight(id));
		...
		public async Task<ActionResult<FlightEntity>> PostFlight(FlightCreateDto flightCreateDto)
		{
			...
			await _cacheService.RemoveByPrefixAsync(CacheKeys.FlightsPrefix);
		...
		public async Task<IActionResult> PutFlight(int id, FlightUpdateDto flightUpdateDto)
		{
			...
			await _cacheService.RemoveAsync(CacheKeys.Flight(id));
			await _cacheService.RemoveByPrefixAsync(CacheKeys.FlightsPrefix);
