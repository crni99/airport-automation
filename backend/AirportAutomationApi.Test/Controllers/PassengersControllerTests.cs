...
namespace AirportAutomationApi.Test.Controllers
{
	public class PassengersControllerTests
	{
		...
		public void Constructor_WhenPageSettingsAreMissing_SetsDefaultMaxPageSize(bool isNullOptions)
		{
			...
			// Act
			var controller = new PassengersController(
				new Mock<IPassengerService>().Object,
				...
				new Mock<ILogger<PassengersController>>().Object,
				options
			);
		...
		public async Task GetPassengers_ReturnsInternalServerError_WhenExceptionThrown()
		{
			...
			_paginationValidationServiceMock
				...
				.Returns((true, pageSize, null));
			_cacheServiceMock
				.Setup(x => x.GetOrCreateAsync<PagedResponse<PassengerDto>>(
					It.IsAny<string>(),
					It.IsAny<Func<Task<PagedResponse<PassengerDto>?>>>(),
					null, null))
			...
			// Act & Assert
			await Assert.ThrowsAsync<Exception>(async () => await _controller.GetPassengers(page, pageSize));
		}
		...
		public async Task GetPassengers_ReturnsCachedData_WhenCacheHit()
		{
			...
			var cachedResponse = new PagedResponse<PassengerDto>(
				new List<PassengerDto> { new PassengerDto { Id = 1, FirstName = "Ognjen", LastName = "Andjelic", UPRN = "1234567890123", Passport = "1234567", Address = "DD 10", Phone = "064" } },
				page, pageSize, 1);
			...
			_passengerServiceMock.Verify(x => x.GetPassengers(It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()), Times.Never);
		}
		...
		public async Task GetPassengers_SetsCache_WhenCacheMiss()
		{
			...
			_mapperMock
				.Setup(m => m.Map<IEnumerable<PassengerDto>>(It.IsAny<IEnumerable<PassengerEntity>>()))
				.Returns(new List<PassengerDto> { new PassengerDto { Id = 1, FirstName = "Ognjen", LastName = "Andjelic", UPRN = "1234567890123", Passport = "1234567", Address = "DD 10", Phone = "064" } });
			...
			_cacheServiceMock.Verify(x => x.GetOrCreateAsync<PagedResponse<PassengerDto>>(
				It.IsAny<string>(),
				It.IsAny<Func<Task<PagedResponse<PassengerDto>?>>>(),
		...
		public async Task GetPassenger_PassengerNotFound_ReturnsNotFound()
		{
			...
			_passengerServiceMock
				...
				.ReturnsAsync((PassengerEntity)null);
		...
		public async Task GetPassenger_ReturnsPassengerDto_WhenPassengerExists()
		{
			...
			_inputValidationServiceMock
				...
				.Returns(true);
			_cacheServiceMock
				.Setup(x => x.GetOrCreateAsync<PassengerDto>(
					It.IsAny<string>(),
					It.IsAny<Func<Task<PassengerDto?>>>(),
					null, null))
				.Returns<string, Func<Task<PassengerDto?>>, TimeSpan?, TimeSpan?>(
					async (key, factory, abs, sld) => await factory());
			_passengerServiceMock
				.Setup(service => service.GetPassenger(validId))
		...
		[Fact]
		[Trait("Category", "GetPassenger")]
		public async Task GetPassenger_ReturnsCachedData_WhenCacheHit()
		{
			// Arrange
			int id = 1;
			var cachedPassenger = new PassengerDto { Id = 1, FirstName = "Ognjen", LastName = "Andjelic", UPRN = "1234567890123", Passport = "1234567", Address = "DD 10", Phone = "064" };
			...
			_cacheServiceMock
				.Setup(x => x.GetOrCreateAsync<PassengerDto>(
					...
					It.IsAny<Func<Task<PassengerDto?>>>(),
					null, null))
				.ReturnsAsync(cachedPassenger);

			// Act
			var result = await _controller.GetPassenger(id);

			// Assert
			var okResult = Assert.IsType<OkObjectResult>(result.Result);
			Assert.Equal(cachedPassenger, okResult.Value);
			_passengerServiceMock.Verify(x => x.PassengerExists(It.IsAny<int>()), Times.Never);
			_passengerServiceMock.Verify(x => x.GetPassenger(It.IsAny<int>()), Times.Never);
		}

		[Fact]
		[Trait("Category", "GetPassenger")]
		public async Task GetPassenger_SetsCache_WhenCacheMiss()
		{
			// Arrange
			int id = 1;
			...
			// Assert
			Assert.IsType<OkObjectResult>(result.Result);
			_cacheServiceMock.Verify(x => x.GetOrCreateAsync<PassengerDto>(
				It.IsAny<string>(),
				It.IsAny<Func<Task<PassengerDto?>>>(),
				null, null), Times.Once);
		}
		...
		public async Task SearchPassengers_ValidFilterWithResults_ReturnsOkWithPagedResponse()
		{
			...
			// Act
			var result = await _controller.SearchPassengers(filter, 1, 10);
		...
		public async Task SearchPassengers_InvalidPagination_ReturnsBadRequestFromValidationService()
		{
			...
			// Act
			var result = await _controller.SearchPassengers(filter, -1, 0);
		...
		public async Task SearchPassengers_PageSizeExceedsMax_CorrectsPageSize()
		{
			...
			// Act
			var result = await _controller.SearchPassengers(filter, 1, 100);
		...
		public async Task ExportToPdf_InvalidPaginationParameters_ReturnsBadRequest()
		{
			...
			// Act
			var result = await _controller.ExportToPdf(new PassengerSearchFilter(), page: -1, pageSize: 0);
		...
		public async Task ExportToExcel_InvalidPaginationParameters_ReturnsBadRequest()
		{
			...
			// Act
			var result = await _controller.ExportToExcel(new PassengerSearchFilter(), page: -1, pageSize: 0);
		...
		public async Task ExportToExcel_GenerationFails_ReturnsInternalServerError()
		{
			...
			// Act
			var result = await _controller.ExportToExcel(new PassengerSearchFilter());
