...
namespace AirportAutomationApi.Test.Controllers
{
	public class AirlinesControllerTests
	{
		...
		public async Task GetAirlines_InvalidPaginationParameters_ReturnsBadRequest()
		{
			...
			// Act
			var result = await _controller.GetAirlines(invalidPage, invalidPageSize, cancellationToken);
		...
		public async Task GetAirlines_ReturnsNoContent_WhenNoAirlinesFound()
		{
			...
			// Act
			var result = await _controller.GetAirlines(page, pageSize, cancellationToken);
		...
		public async Task GetAirlines_ReturnsInternalServerError_WhenExceptionThrown()
		{
			...
			_paginationValidationServiceMock
				...
				.Returns((true, pageSize, null));
			_cacheServiceMock
			   .Setup(x => x.GetOrCreateAsync<PagedResponse<AirlineDto>>(
				   It.IsAny<string>(),
				   It.IsAny<Func<Task<PagedResponse<AirlineDto>?>>>(),
				   null, null))
			...
			// Act & Assert
			await Assert.ThrowsAsync<Exception>(async () => await _controller.GetAirlines(page, pageSize, cancellationToken));
		}
		...
		/// <summary>
		/// This test verifies that the `GetAirlines` method returns a `200 OK`
		/// response with cached data when the cache contains the requested airlines.
		/// </summary>
		[Fact]
		[Trait("Category", "GetAirlines")]
		public async Task GetAirlines_ReturnsCachedData_WhenCacheHit()
		{
			...
			var cachedResponse = new PagedResponse<AirlineDto>(
				new List<AirlineDto> { new AirlineDto { Id = 1, Name = "Air Serbia" } },
				page, pageSize, 1);
			...
			_airlineServiceMock.Verify(x => x.GetAirlines(It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()), Times.Never);
		}

		/// <summary>
		/// This test verifies that the `GetAirlines` method calls SetAsync on the cache
		/// after successfully retrieving airlines from the database.
		/// </summary>
		[Fact]
		[Trait("Category", "GetAirlines")]
		public async Task GetAirlines_SetsCache_WhenCacheMiss()
		{
			...
			_cacheServiceMock.Verify(x => x.GetOrCreateAsync<PagedResponse<AirlineDto>>(
				It.IsAny<string>(),
				It.IsAny<Func<Task<PagedResponse<AirlineDto>?>>>(),
		...
		[Fact]
		[Trait("Category", "GetAirline")]
		public async Task GetAirline_ReturnsCachedData_WhenCacheHit()
		{
			// Arrange
			int id = 1;
			var cachedAirline = new AirlineDto { Id = 1, Name = "Air Serbia" };
			...
			_cacheServiceMock
				.Setup(x => x.GetOrCreateAsync<AirlineDto>(
					...
					null, null))
				.ReturnsAsync(cachedAirline);

			// Act
			var result = await _controller.GetAirline(id);

			// Assert
			var okResult = Assert.IsType<OkObjectResult>(result.Result);
			Assert.Equal(cachedAirline, okResult.Value);
			_airlineServiceMock.Verify(x => x.AirlineExists(It.IsAny<int>()), Times.Never);
			_airlineServiceMock.Verify(x => x.GetAirline(It.IsAny<int>()), Times.Never);
		}

		[Fact]
		[Trait("Category", "GetAirline")]
		public async Task GetAirline_SetsCache_WhenCacheMiss()
		{
			// Arrange
			int id = 1;
			...
			// Assert
			Assert.IsType<OkObjectResult>(result.Result);
			_cacheServiceMock.Verify(x => x.GetOrCreateAsync<AirlineDto>(
				It.IsAny<string>(),
				It.IsAny<Func<Task<AirlineDto?>>>(),
				null, null), Times.Once);
		}
		...
		public async Task SearchAirlines_InvalidPaginationParameters_ReturnsBadRequest()
		{
			...
			// Act
			var result = await _controller.SearchAirlines(validName, invalidPage, invalidPageSize, cancellationToken);
		...
		public async Task SearchAirlines_ReturnsPagedListOfAirlines_WhenAirlinesFound()
		{
			...
			// Act
			var result = await _controller.SearchAirlines(validName, validPage, validPageSize, cancellationToken);
