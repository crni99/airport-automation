...
namespace AirportAutomation.Web.Controllers
{
	...
	public class PilotController : BaseController
	{
		...
		public PilotController(IDataHttpService dataHttpService, ISearchHttpService searchHttpService, IExportHttpService exportHttpService, IAlertService alertService, IMapper mapper)
		{
		...
		public async Task<IActionResult> GetPilots(int page = 1, int pageSize = 10, CancellationToken cancellationToken = default)
		{
			...
			var response = await _dataHttpService.GetDataList<PilotEntity>(page, pageSize, cancellationToken);
			if (response == null)
			{
		...
		{
			var response = await _dataHttpService.GetData<PilotEntity>(id, cancellationToken);
			if (response is null)
			{
			...
			CancellationToken cancellationToken = default)
		{
			...
			var response = await _searchHttpService.GetDataByFilter<PilotEntity>(filter, page, pageSize, cancellationToken);
			if (response == null)
			{
				return Json(new { success = false, message = "No pilots found." });
			}
		...
		public async Task<IActionResult> CreatePilot(PilotCreateViewModel pilotCreateDto, CancellationToken cancellationToken = default)
		{
			if (ModelState.IsValid)
			{
				...
				var response = await _dataHttpService.CreateData<PilotEntity>(pilot, cancellationToken);
				if (response is null)
				{
		...
		public async Task<IActionResult> EditPilot(PilotViewModel pilotDto, CancellationToken cancellationToken = default)
		{
			if (ModelState.IsValid)
			{
				...
				var response = await _dataHttpService.EditData<PilotEntity>(pilot, pilot.Id, cancellationToken);
				if (response)
				{
		...
		{
			var response = await _dataHttpService.DeleteData<PilotEntity>(id, cancellationToken);
			if (response)
			{
		...
		public async Task<IActionResult> DownloadFile(
			...
			CancellationToken cancellationToken = default)
		{
			var result = await _exportHttpService.DownloadFileAsync<PilotEntity>(fileType, filter, page, pageSize, getAll, cancellationToken);
